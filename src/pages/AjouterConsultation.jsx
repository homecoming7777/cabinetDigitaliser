...
export default function AjouterConsultation() {
  ...
  return (
    ...
    <form onSubmit={submit} className="bg-[#2f404f9d] mb-10 mt-10 rounded-lg p-6 space-y-4 shadow-2xl">
      ...
      <input required className="border-2 rounded-lg outline-0 w-full p-2 border-white text-white" name="patient" placeholder="Patient" onChange={handleChange} />
      ...
      <input required className="border-2 rounded-lg outline-0 w-full p-2 border-white text-white" name="diagnostic" placeholder="Diagnostic" onChange={handleChange} />

      <select required className="border-2 rounded-lg outline-0 w-full p-2 border-white text-white" name="type" onChange={handleChange}>
      ...
      <button  className="group text-white relative inline-flex h-12 items-center justify-center overflow-hidden rounded-md border border-neutral-200 bg-transparent px-6 font-medium transition-all [box-shadow:0px_4px_1px_#a3a3a3] active:translate-y-[2px] active:shadow-none" type="submit">Créer</button>
    </form>
    </div>
    </>
  );
